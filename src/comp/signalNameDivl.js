    ...
    return(
        ...
        >
            {signals.map((signal, index) => (
                (Object.keys(signal).includes('name'))&&(<text className="signal-label" x={offsetX - signal.name.length * 9 - 12} y={index * (dy + offsetY) + 1.0*dy} textAnchor="left" fill="blue" key={index}> {signal.name} </text>)
            ))}
        </svg>
    );
